...
- Set `voice.realtime.provider: "codex"` to attach native Codex Realtime V3 to the routed Discord agent session. This provider runs that bound Codex thread directly, accepts audio only from an authorized owner, and does not add a second `openclaw_agent_consult`; V3 uses the ChatGPT subscription selected by the agent-scoped Codex app-server auth. V1 and V2 retain OpenAI Platform API-key auth.
